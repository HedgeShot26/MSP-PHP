            ...
            <div class="col-lg-10 mt-5 mt-lg-5">
                <div class="container" style="border:10px solid #eeeeee; padding: 20px; border-radius:50px; box-shadow: 0 8px 18px rgba(8, 8, 8, 0.3); ">
                    ...
                    <div class="mb-5" style="border-radius: 30px;background-color: #07689F; color: white; text-transform: uppercase; font-weight: bold;">
                        <h3 class="mt-1 py-1">
                            <span class="ps-4 ">Edit Sales</span>
        ...
        <div class="row justify-content-md-center">
            <div class="col-lg-10 mt-lg-3">
                <div class="container" style="border:10px solid #eeeeee; padding: 20px; border-radius:50px; box-shadow: 0 8px 18px rgba(8, 8, 8, 0.3); ">
                    ...
                    <form action="<?= base_url('Sales/confirm_edit_sale_items/'.$sales_id) ?>" method="post" enctype="multipart/form-data">
                        <?= csrf_field(); ?>
                        ...
                        <table class="table box-table table-hover table-borderless" id="addsalestable">
                            ...
                            <tbody id="body">
                                <?php
                                $sales_item_num = count($sale_pro_data);
                                $counter = 1;
                                ?>
                                <script>
                                    var i = <?= $sales_item_num ?>;
                                </script>
                                <?php foreach ($sale_pro_data as $row) { ?>
                                    <tr id="row<?=$counter?>" class="tb-body table-shadow ">
                                        <td style="width:15%;"><input type="number" name="product_id[]" class="form-control product_id" value="<?=$row['Product_id']?>" readonly /></td>
                                        <td style="width:40%;"><input type="text" name="product_name[]" class="form-control" value="<?=$row['Product_Name']?>" readonly /></td>
                                        <td style="width:17%;"><input type="number" name="product_price[]" class="form-control" value="<?=$row['Product_id']?>" readonly /></td>
                                        <td><input type="number" name="product_quantity[]" class="form-control" min="1" max="<?=$row['Product_Quantity']?>" value="<?=$row['SPro_Quantity']?>" /></td>
                                        <td style="width:10%;"><input type="number" class="form-control" value="<?=$row['Product_Quantity']?>" readonly /></td>
                                        <td style="width:2%;">
                                            <button type="button" style="width:3em;" name="remove" id="<?=$counter?>" class="btn btn-danger btn_remove"><span class="fas fa-trash"></span></button>
                                        </td>
                                    </tr>
                                <?php $counter++;
                                } ?>
